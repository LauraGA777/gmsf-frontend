...
import { User, Plus, Trash2, Users, ShieldCheck, AlertTriangle } from "lucide-react";
...
const beneficiarySchema = z.object({
  ...
  usuario: z.object({
    ...
    fecha_nacimiento: z.string().refine(
      (date) => new Date(date) < new Date(), // Basic validation: not in future
...
export const updateClientSchema = z.object({
  usuario: z.object({
    ...
    // Campos que faltaban y son parte del formulario de edición, aunque no se puedan cambiar
    tipo_documento: z.enum(['CC', 'CE', 'TI', 'PP', 'DIE']),
    numero_documento: z.string().min(5, "El número de documento debe tener al menos 5 caracteres"),
    fecha_nacimiento: z.string().refine(
...
// Ensure client.usuario is not null for this modal
  ...
  const {
    ...
    defaultValues: {
      ...
      contactos_emergencia: (client.contactos_emergencia || []).map(c => ({
        ...c,
        es_mismo_beneficiario: c.es_mismo_beneficiario ?? false,
      })),
      beneficiarios: ((client.beneficiarios as any[]) || []).map(b => ({
        id: b.id_persona,
        usuario: {
  ...
  return (
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-4xl max-h-[95vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit, (formErrors: FieldErrors<UpdateClientFormValues>) => {
          ...
          if (formErrors.contactos_emergencia) {
            errorMessage = 'Se requiere al menos un contacto de emergencia.';
          } else if (formErrors.usuario?.fecha_nacimiento) {
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <Select onValueChange={(val) => setValue("estado", val === 'true')} value={String(watchedEstado)}>
            ...
            <TabsContent value="beneficiarios">
              <Card>
                ...
                <CardContent className="space-y-3">
                  ...
                  <Button type="button" variant="outline" size="sm" className="mt-2" onClick={() => appendBeneficiary(defaultBeneficiaryValue)}>
